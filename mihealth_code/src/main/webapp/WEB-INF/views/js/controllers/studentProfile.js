...
angular.module('mihealth', [
    ...
    'ngTable',
    'ngFileUpload'
]).controller('studentProfileCtrl', function($scope, $http, $filter, $uibModal, $sce, $state, $location, ngTableParams, Upload, growlService, propertyService, localeService) {
    //Edit
    this.editInfo = 0;
    this.editContact = 0;
    ...
    $scope.init = function(){
		...
		+]*/
    	$scope.userUid = $state.params.uid;
	    loadStudentInfo($scope.userUid);
	}
    
    var loadStudentInfo = function(userUid){
	    if (!angular.isUndefined(userUid)){
		    $http.get('api/david/student?uid='+userUid).success(function(data) {
	    		$scope.student = data.data;
	    		$scope.profileUrl = getProfileUrl(); 
	    		
	    		$scope.reloadMeasurementData();
	    		$scope.reloadDiseaseData();
			}).error(function(error) {
		  	});
	    }
    }
	
    $scope.reloadMeasurementData = function(){
    	$scope.measurementTable.reload();
    }

    $scope.reloadDiseaseData = function(){
    	$scope.diseaseTable.reload();
    }
    
	$scope.measurementTable = new ngTableParams({
        ...
        getData: function($defer, params) {
        	if (!angular.isUndefined($scope.student)){
	        	var urlParams = $.param({
	                orderBy : angular.toJson(params.sorting()),
	                filter : function(){
	                	var filter = params.filter();
                		filter['userUid'] = $scope.student.userUid;
	        	...
	        	$http.get('api/campus/measurement?'+urlParams).success(function(data) {
	...
	$scope.diseaseTable = new ngTableParams({
        page: 1,            // show first page
        count: 10,           // count per page
        sorting: {registeredAt:'desc'}
    }, {
        ...
        getData: function($defer, params) {
        	if (!angular.isUndefined($scope.student)){
	        	...
	        	$http.get('api/campus_id/care?'+urlParams).success(function(data) {
	        		var records = data.data;
	        		params.total(records.length);
	                $defer.resolve(records.slice((params.page() - 1) * params.count(), params.page() * params.count()));
				}).error(function(error) {
    ...
    this.save = function(user) {            
    	...
    	$http.post('api/'+user.campusUid+'/student/save', user, config).success(function(data) {
 ...
 // Account Table
	...
	this.getDiseaseName = function(deptId, diseases){
